name_module_singular = "Estado";
id_form = "#statusForm";
base_url_module = base_url + "/status";
...
$(document).ready(function () {
  ...
  let table = $("#zero_config").DataTable({
    ...
    columns: [
      { title: "id", data: "idEstado" },
      { title: "Nombre", data: "nombre" },
      { title: "Descripción", data: "descripcion" },
      {
        title: "Solapamiento",
        data: "considerar_solapamiento",
      ...
      },
      {
        ...
        data: "color",
        render: function (data, type, row) {
          return `<div style="width: 40px; height: 30px; background-color: ${data}; border: 1px solid #aaa;"></div>`;
        },
      },
       {
        data: null,
  ...
  $("body").on("click", ".mdl-edit-btn", function (event) {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        $("#idEstado").val(response.statu.idEstado);
        $("#nombre").val(response.statu.nombre);
        $("#descripcion").val(response.statu.descripcion);
        $("#color").val(response.statu.color);
        $("#considerar_solapamiento").prop('checked', response.statu.considerar_solapamiento == 1);
