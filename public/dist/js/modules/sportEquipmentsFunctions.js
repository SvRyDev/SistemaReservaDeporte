name_module_singular = "Implemento Deportivo";
id_form = "#sportEquipmentForm";
base_url_module = base_url + "/sportEquipments";
...
$(document).ready(function () {
  ...
  let table = $("#zero_config").DataTable({
    ...
    columns: [
      { title: "id", data: "idImplemento" },
      { title: "Nombre", data: "nombre" },
      { title: "Descripción", data: "descripcion" },
      { title: "Precio de Alquiler (S:/)", data: "precioAlquiler" },
      {
        title: "Disponible",
        data: "estado",
...

      {
        ...
        render: function (data, type, row) {
          return `
                            <button type="button" class="btn btn-warning text-white mdl-edit-btn btn-sm" data-id="${data.idImplemento}" data-bs-toggle="modal" data-bs-target=""><span class="span_icon_edit">${icon_edit}</span><span class="d-none d-lg-inline">Editar</span></button>
                            <button type="button" class="btn btn-danger text-white mdl-delete-btn btn-sm" data-id="${data.idImplemento}" >${icon_delete}<span class="d-none d-lg-inline">Eliminar</span></button>
                        `;
        },
      },
    ],
  ...
  $("body").on("click", ".mdl-edit-btn", function (event) {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        $("#idImplemento").val(response.equipment.idImplemento);
        $("#nombre").val(response.equipment.nombre);
        $("#descripcion").val(response.equipment.descripcion);
        $("#precioAlquiler").val(response.equipment.precioAlquiler);
        $("#estado").prop('checked', response.equipment.estado == 1);
