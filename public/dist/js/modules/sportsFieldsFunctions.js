id_form = "#sportsFieldsForm";
name_module_singular = "Campo";
base_url_module = base_url + "/sportsFields";
...
$(document).ready(function () {
  ...
  let table = $("#zero_config").DataTable({
    ...
    columns: [
      { title: "id", data: "idCampo" },
      { title: "Nombre", data: "codigo" },
      { title: "Tipo de Deporte", data: "tipo_deporte"},
      { title: "Alquiler por Hora (S/.)", data: "alquilerHora" },
      {
        title: "Disponible",
        data: "disponible",
 ...
 
      {
        ...
        render: function (data, type, row) {
          return `
                            <button type="button" class="btn btn-warning text-white mdl-edit-btn btn-sm" data-id="${data.idCampo}" data-bs-toggle="modal" data-bs-target=""><span class="span_icon_edit">${icon_edit}</span><span class="d-none d-lg-inline">Editar</span></button>
                            <button type="button" class="btn btn-danger text-white mdl-delete-btn btn-sm" data-id="${data.idCampo}" >${icon_delete}<span class="d-none d-lg-inline">Eliminar</span></button>
                        `;
        },
      },
    ],
  ...
  $("body").on("click", ".mdl-edit-btn", function (event) {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        $("#idCampo").val(response.field.idCampo);
        $("#codigo").val(response.field.codigo);
        $("#idTipoDeporte").val(response.field.idTipoDeporte);
        $("#alquilerHora").val(response.field.alquilerHora);
        $("#disponible").prop('checked', response.field.disponible == 1);
  ...
  function deleteData(dataId) {
    ...
    $('#alquilerHora').on('wheel', function(event) {
        // Prevenir el comportamiento de scroll de la página
        event.preventDefault();

        // Obtener el valor actual del input
        let currentValue = parseFloat($(this).val()) || 0;

        // Incrementar o decrementar el valor basado en la dirección del scroll
        if (event.originalEvent.deltaY < 0) {
            // Scroll hacia arriba, incrementar el valor
            currentValue += .5;
        } else {
            // Scroll hacia abajo, decrementar el valor
            currentValue -= 0.5;
        }
