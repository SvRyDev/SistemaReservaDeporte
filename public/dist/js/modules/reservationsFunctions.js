...
name_module_singular = "Reserva";
id_form = "#reservationForm";
base_url_module = base_url + "/reservations/";
...
$(document).ready(function () {
  //█ READ ALL DATA - Inicializa DataTable y guarda la referencia
  let table = $("#zero_config").DataTable({
    ...
    columns: [
      { title: "id", data: "idReserva" },
      { title: "Cliente", data: "cliente_nombre" },
      { title: "Campo", data: "campo_codigo" },
      { title: "Hora Ingreso", data: "fechaEntrada" },
      { title: "Hora Salida", data: "fechaSalida" },
      { title: "Precio", data: "precioTotal" },
      {
        title: "Estado de Reserva",
        data: null,
        render: function (data, type, row) {
          return `<span class="badge rounded-pill" style="background-color: ${row.estado_color}; border: 0px solid #aaa;">${row.estado_nombre}</span>`;
        },
      },
      { title: "Estado de Pago", data: "estado_pago" },

      {
        data: null,
        render: function (data, type, row) {
          return `
                                                                        <a class="btn btn-warning btn-sm" href="${base_url}/reservations/edit/${data.encrypted_id}; ?>"><i class="far fa-edit"></i> Editar</a>
                                                                        <a class="btn btn-success btn-sm text-white" href="${base_url}/payments/create/${data.idReserva}"><i class="fas fa-dollar-sign"></i> Realizar Pago</a>
                                                                        <a class="btn btn-info btn-sm" href="${base_url}/reservations/updateState/${data.idReserva}"><i class="mdi mdi-refresh"></i> Actualizar Estado</a>
                                                                        <a class="btn btn-secondary btn-sm" href="${base_url}/reservations/viewPayments/${data.idReserva}"><i class="mdi mdi-cash-multiple"></i> Ver Pagos</a>
                                                                        <a class="btn btn-danger btn-sm" href="${base_url}/reservations/delete/${data.idReserva}"><i class="fas fa-trash-alt"></i> Eliminar</a>
                        `;
        },
      },
    ],
  ...
  $("body").on("click", ".mdl-add-btn", function () {
    ...
    $("#header-modal-form")
      .removeClass("bg-info-gradient")
      .addClass("bg-success-gradient");
  ...
  //█ LOAD DATA BY ID - Inicializar modal y cargar los datos (Para actualizar)
  $("body").on("click", ".mdl-edit-btn", function (event) {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        $("#idTipoDeporte").val(response.sport.idTipoDeporte);
        $("#nombre").val(response.sport.nombre);
        $("#descripcion").val(response.sport.descripcion);
        ...
        $("#header-modal-form")
          .removeClass("bg-success-gradient")
          .addClass("bg-info-gradient");
        $("#eventModalLabel").html(
          icon_feather + " Editar " + name_module_singular
        );
      ...
      error: function (xhr, status, error) {
        console.error(
          "Error al cargar los datos de " + name_module_singular + ":",
          error
        );
        ...
        $("#loadingSpinner").hide();
        alert(
          "Hubo un error al cargar los datos de " +
  ...
  $(id_form).on("submit", function (event) {
    ...
    $.ajax({
      ...
      error: function (xhr, status, error) {
        console.error(
          "Error al actualizar los datos de " + name_module_singular + ":",
          error
        );
        alert(
...
function fetchReservationDetails(url) {
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      $('#desc-info-reserve').css('background-color', response.details.estado_color);
      console.log(response.details.estado_color);
      $("#eventModalLabel").text("Detalles de la Reserva");
      $("#eventModalLabel").css("color", "white");
      $("#eventDetails").html(
        `
        <p>Estado de Reserva: <span class="badge rounded-pill" style="background-color: ${response.details.estado_color}; "> ${response.details.estado_nombre} </span></p>
        <label>Cliente:</label><span class="form-control"> ${response.details.cliente_nombre}</span>
         <label>Fecha de Entrada:</label>  <span class="form-control">${response.details.fechaEntrada}</span>
         <label>Fecha de Salida:</label>  <span class="form-control">${response.details.fechaSalida}</span>
         <label>Detalle:</label>  <span class="form-control">${response.details.detalle}</span>
         <label>Precio Total:</label>  <span class="form-control">${response.details.precioTotal}</span>
         `
      );
      ...
      response.details.implementos.forEach(function (implemento) {
        implementosList.append(
          `<li>${implemento.nombre} - Cantidad: ${implemento.Cantidad}, Precio Total: ${implemento.PrecioTotal}</li>`
        );
      });
