name_module_singular = "Metodo de Pago";
id_form = "#paymentMethodForm";
base_url_module = base_url + "/paymentMethods";
...
$(document).ready(function () {
  ...
  let table = $("#zero_config").DataTable({
    ...
    columns: [
      { title: "id", data: "idMetodoPago" },
      { title: "Nombre", data: "nombre" },
      { title: "Codigo", data: "codigo" },
      { title: "Descripción", data: "notas" },
      { title: "Tarifa (S:/)", data: "tarifa_adicional" },
      { title: "Url", data: "url_integracion" },
      {
        title: "Disponible",
        data: "disponible",
 ...
 
      {
        ...
        render: function (data, type, row) {
          return `
                            <button type="button" class="btn btn-warning text-white mdl-edit-btn btn-sm" data-id="${data.idMetodoPago}" data-bs-toggle="modal" data-bs-target=""><span class="span_icon_edit">${icon_edit}</span><span class="d-none d-lg-inline">Editar</span></button>
                            <button type="button" class="btn btn-danger text-white mdl-delete-btn btn-sm" data-id="${data.idMetodoPago}" >${icon_delete}<span class="d-none d-lg-inline">Eliminar</span></button>
                        `;
        },
      },
    ],
  ...
  $("body").on("click", ".mdl-edit-btn", function (event) {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        $("#idMetodoPago").val(response.paymentMethod.idMetodoPago);
        $("#nombre").val(response.paymentMethod.nombre);
        $("#notas").val(response.paymentMethod.notas);
        $("#codigo").val(response.paymentMethod.codigo);
        $("#tarifa_adicional").val(response.paymentMethod.tarifa_adicional);
        $("#disponible").prop('checked', response.paymentMethod.disponible == 1);
        $("#url_integracion").val(response.paymentMethod.url_integracion);
